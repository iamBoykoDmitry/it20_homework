...
SELECT `users`.`surname`, `users`.`name`, `areas`.`name` FROM `areas` LEFT JOIN `users` ON `users`.`area_id` = `areas`.`id` ORDER BY `areas`.`name` LIMIT 4;

3. Необходимо посчитать сколько новостей в каждой категории. Связь `news`.`category_id` = `categories`.`id`.

SELECT `categories`.`name`, COUNT(`news`.`id`) FROM `categories` LEFT JOIN `news` ON `news`.`category_id` = `categories`.`id` GROUP BY `categories`.`id`;
...
SELECT `cities`.`name`, `areas`.`name` FROM `areas` LEFT JOIN `cities` ON `cities`.`area_id` = `areas`.`id`;
...
SELECT `schools`.`name`, `districts`.`name` FROM `schools` RIGHT JOIN `districts` ON `schools`.`district_id` = `districts`.`id`